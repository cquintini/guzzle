...
Guzzle relies on the ``Guzzle6Http/psr7`` Composer package for its message
implementation of PSR-7.

You can create a request using the ``Guzzle6Http\Psr7\Request`` class:
...
You can create a response using the ``Guzzle6Http\Psr7\Response`` class:
...
.. code-block:: php
    ...
    $response = Guzzle6Http\get('http://httpbin.org/get');
...
method from the ``Guzzle6Http\Psr7\Utils`` class --
...
.. code-block:: php
    ...
    $request = new \Guzzle6Http\Psr7\Request('MOVE', 'http://httpbin.org/move');
...
Guzzle uses the ``Guzzle6Http/psr7`` package to provide stream support. More
...
The best way to create a stream is using the ``Guzzle6Http\Psr7\Utils::streamFor``
