...
.. warning::
    ...
    when it comes to the node.js test server or the ``Guzzle6Http\Tests\Server``
    class. If you are using the test server or ``Server`` class outside of
    Guzzle6Http/guzzle, then you will need to configure autoloading and
...
``Guzzle6Http\Tests\Server`` provides a static interface to the test server. You
